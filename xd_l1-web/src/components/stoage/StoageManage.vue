<template>
  <div class="index" style="height: 100%">
    <div style="margin-bottom: 5px">
      <el-input v-model="name" placeholder="请输入仓库" suffix-icon="el-icon-search" style="width: 200px"
      ...
      >
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
          <el-form-item label="仓库名" prop="name" style="width: 80%">
...
export default {
  ...
  methods:{
    ...
    del(id){
      console.log(id)
      this.$axios.post(this.$httpUrl+'/stoage/del?id='+id,this.ruleForm).then(res=>res.data).then(res=>{
        console.log(res)
        if (res.code==200) {
          this.$message({
            message: '删除成功',
    ...
    doSave(){
      this.$axios.post(this.$httpUrl+'/stoage/save',this.ruleForm).then(res=>res.data).then(res=>{
        console.log(res)
        if (res.code==200) {
          this.$message({
            message: '添加成功',
    ...
    doMod(){
      this.$axios.post(this.$httpUrl+'/stoage/update',this.ruleForm).then(res=>res.data).then(res=>{
        console.log(res)
        if (res.code==200) {
          this.$message({
            message: '添加成功',
    ...
    //显示数据
    loadPost(){
      this.$axios.post(this.$httpUrl+'/stoage/listPage',{
